...
async function checkItServiceRenewalReminders() {
  ...
  try {
    ...
    const itTeamEmails = users
      .filter(u => u && u.active !== false && u.email && (u.perms?.admin || u.perms?.itServiceRenewalManage))
      ...
      if (newlyCrossedThresholds.length) {
        ...
        await insertSystemLog({
          ...
          description: recipients.length
            ...
            : `Dịch vụ CNTT [${item.name}] (${label}) chưa có người nhận hợp lệ (chưa ai có quyền itServiceRenewalManage kèm email hợp lệ)`,
