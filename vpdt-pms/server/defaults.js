...
const DEFAULTS = {
  ...
  // meetings (Bước 6c/6d/6e/6f/6g/6h) KHÔNG còn ở đây — lưu ở bảng riêng (dbo.SystemLogs, dbo.Tasks,
  // dbo.Records — xem lib/systemLogStore.js, lib/taskStore.js, lib/recordStore.js), không còn là 1
  // dòng JSON trong AppData. routes/data.js không nhận các key này làm key hợp lệ nữa (loại khỏi
  // VALID_KEYS tự nhiên vì không còn trong DEFAULTS) — systemLogs dùng POST/DELETE /api/log
  ...
  // (routes/meetingActions.js) — mỗi collection tiếp theo được thêm vào lib/recordStore.js
  // MIGRATED_COLLECTIONS sẽ tự động theo đúng mẫu này (miễn route đọc/ghi của nó đã đi qua
  // getAllForCollection/createForCollection/withLockedRecordForCollection), không cần sửa gì thêm ở
  // routes/data.js.
};
