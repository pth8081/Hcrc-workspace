...
async function main() {
  ...
  try {
    ...
    results = await page.evaluate(async () => {
      ...
      check('muaHang: sidebar button visible for admin', !document.getElementById('btnMuaHangTab').classList.contains('hidden'));
    ...
    results = await page.evaluate(async () => {
      ...
      check('muaHang: sidebar button hidden for user without any rebate perm', document.getElementById('btnMuaHangTab').classList.contains('hidden'));
      alerts.length = 0;
