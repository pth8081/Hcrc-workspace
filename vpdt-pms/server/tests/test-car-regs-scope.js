...
async function main() {
  ...
  try {
    ...
    await run.run('dù nhánh tải trả THỪA (giả lập lỗi tầng dưới), filterCarRegsForUser() vẫn chốt đúng phạm vi (lớp chắn thứ 2)', async () => {
      ...
      stubModule('lib/recordStore', {
        MIGRATED_COLLECTIONS: new Set(['carRegs']),
        getAllForCollectionCached: async () => ALL_CARS.map(r => ({ ...r })),
        getForCollectionByDeptCached: async () => ALL_CARS.map(r => ({ ...r })), // CỐ Ý trả thừa mọi phòng ban
        getForCollectionByUsernameCached: async () => [],
        getForCollectionByColumnCached: async () => ALL_CARS.map(r => ({ ...r })) // CỐ Ý trả thừa
      });
