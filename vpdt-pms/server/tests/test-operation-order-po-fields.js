...
// orderLocationType (STORE/HO) — MỚI BẮT BUỘC từ đợt "Tách Đơn Hàng Siêu Thị/HO" (không liên quan gì
// tới đợt PDF-autofill mà file này kiểm tra) — mọi payload dưới đây phải có field này, nếu không
// validateAndPrepareCreate() sẽ ném lỗi 400 "Vui lòng chọn đúng nơi đặt hàng" trước khi chạm tới bất kỳ
// field PO nào. Giá trị STORE/HO cụ thể không ảnh hưởng gì tới các assertion của file này (chỉ kiểm tra
// field PO/items, không kiểm tra quy trình duyệt).
...
run('operationOrders: discountAmount/vatAmount số nguyên round-trip đúng, chuỗi lạ tự về 0 an toàn', () => {
  const payload = {
    code: 'VH-DH-PDF-002', orderLocationType: 'HO', title: 'Test parse số', supplier: 'NCC X',
...
run('operationOrders: discountAmount âm bị ép về 0 (Math.max(0, ...))', () => {
  const payload = {
    code: 'VH-DH-PDF-003', orderLocationType: 'HO', title: 'Test số âm', supplier: 'NCC Y',
    items: [{ name: 'Hàng test', unit: 'Cái', qty: 1, unitPrice: 1000 }],
    discountAmount: -5000
  };
  const record = validateAndPrepareCreate('operationOrders', payload, USER, [], {}, []);
...
run('operationOrders: orderDate/deliveryDate sai định dạng tự về rỗng, KHÔNG throw', () => {
  const payload = {
    code: 'VH-DH-PDF-004', orderLocationType: 'HO', title: 'Test ngày sai', supplier: 'NCC Z',
    items: [{ name: 'Hàng test', unit: 'Cái', qty: 1, unitPrice: 1000 }],
