...
function addDeductionStoreSheet(wb, sheetName, template, submissions) {
  const sheet = wb.addWorksheet(sheetName);
  sheet.columns = [
    { header: 'Ngày kiểm tra', width: 16 }, { header: 'Người kiểm tra', width: 20 },
  ...
  submissions.forEach((sub, subIdx) => {
    ...
    const summaryRow = sheet.addRow([
      guard(sub.submittedAt), guard(sub.submittedByName), '', '', '',
...
function buildDeductionReportWorkbook(template, submissionsByStore) {
  ...
  for (const [storeCode, submissions] of submissionsByStore.entries()) {
    ...
    addDeductionStoreSheet(wb, sheetName, template, submissions);
  }
  if (!wb.worksheets.length) wb.addWorksheet('Không có dữ liệu');
  return wb;
}

module.exports = { buildQaReportWorkbook, buildDeductionReportWorkbook, safeSheetName };
