...
const CREATE_MODULE_CONFIGS = {
  ...
  operationStoreOpenings: {
    ...
    extraValidate: (payload, collection, user, appData) => {
      ...
      // field DUY NHẤT còn lại cho ngân sách của hồ sơ (đợt gỡ bỏ hẳn field cũ "Chi Phí Phê Duyệt"
      // estimatedBudget — 2 field từng trùng lặp gây nhầm lẫn, xem lịch sử ở git log). Bắt buộc nhập vì
      // đây chính là con số Danh Mục Đầu Tư dùng để tính "còn lại" — để trống/0 vô nghĩa cho mục đích
      // này. Hồ sơ CŨ trước bản vá này KHÔNG có field — đọc lại sẽ là undefined/null, các nơi hiển thị
      // (renderOperationEstimateList()...) tự xử lý hiện "(chưa nhập)" thay vì 0/NaN.
  ...
  operationRepairs: {
    ...
    extraValidate: (payload, collection, user, appData) => {
      ...
      payload.supplier = String(payload.supplier || '').trim();
      // "Ngân Sách Phê Duyệt" — cùng field mới/lý do đã thêm ở operationStoreOpenings ngay trên (đợt gỡ
      // bỏ hẳn field cũ "Chi Phí Phê Duyệt" amount, cùng đợt với estimatedBudget ở operationStoreOpenings).
      if (payload.approvedBudget === undefined || payload.approvedBudget === null || payload.approvedBudget === '') {
        throw new CreateError(400, 'Vui lòng nhập Ngân sách phê duyệt (dùng để tính Ngân sách còn lại ở Danh mục đầu tư)');
      }
      const approvedBudget = Number(payload.approvedBudget);
