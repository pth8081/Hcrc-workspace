...
const REPORT_MODULE_CONFIGS = {
  ...
  budget: {
    ...
    getRecords: (dept, from, to) => fetchReportRecords('budgetLines', dept, from, to,
      () => DB.budgetLines.filter(r => (!dept || r.dept === dept) && isInDateRange(r.createdAt, from, to))),
    statusOf: r => r.status,
    statusBuckets: [['SUBMITTED', 'Chờ duyệt', 'bg-yellow-500'], ['APPROVED', 'Đã duyệt', 'bg-green-500'], ['REJECTED', 'Từ chối', 'bg-red-500']]
  },
  itSupport: {
