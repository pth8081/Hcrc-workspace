...
const CORE_FIELD_MANIFEST = {
  ...
  // (prEntryPeriodSelect) và 2 radio "Hình thức nộp" (prEntryMode) KHÔNG đưa vào — chọn kỳ là chọn 1 bản
  // ghi có sẵn (không phải nhãn tự do), còn 2 hình thức PPTX/PDF gắn trực tiếp 2 luồng xử lý khác hẳn
  // nhau ở client (parsePptxToSlideContents() JSZip vs ghép PDF bằng pdf-lib) — cùng lý do loại trừ
  // paymentSourceType/prEntryMode ở trên.
  REPORT_ENTRY: [
    { id: 'prEntryTitle', label: 'Tiêu Đề Báo Cáo', required: false },
    { id: 'prEntryPptxFile', label: 'Tệp Báo Cáo (.pptx)', required: false },
...
const BUSINESS_MODULES = [
  ...
  // "Báo Cáo Định Kỳ" — quyền pReportManage/pReportAggregate/pReportEntryCreate nằm ở khối 13.
  ...
  // không thấy/vào được. KHÔNG khai vào MODULE_TAB_MAP: module chỉ có đúng 1 tab con và chính
  // nhanSuManage vừa gác module vừa gác tab, không có khái niệm "quyền vào tab" tách biệt.
...
function finishLogin(user) {
  ...
  document.getElementById('btnHrTab').classList.toggle('hidden', !canAccessHrModule(user));
...
function _dispatchTabRender(tabName) {
  ...
  if (tabName === 'hr') { setHrSubTab(activeHrSubTab); }
}
...
// Nhân Sự — module TOP-LEVEL (không có module cha), cùng khuôn canAccessBudgetModule() ở trên. Từ khi
// thêm tab con "Cơ Cấu Tổ Chức" (orgChartManage), module không còn CHỈ 1 quyền chi tiết nữa — vào được
// module khi có BẤT KỲ quyền nào trong 2 tab con (nhanSuManage cho Phản Hồi Ý Kiến, orgChartManage cho
// Cơ Cấu Tổ Chức); mỗi tab con tự gác riêng ở canAccessHrSubTab() bên dưới. Lưu ý: tab "HCRC Đồng Hành"
// bên Truyền Thông (nơi nhân viên GỬI câu hỏi) KHÔNG dùng hàm này — mở cho mọi người.
function canAccessHrModule(user) {
  if (!user) return false;
  if (user.perms?.admin) return true;
  if (!hasModuleAccess(user, 'hr')) return false;
  return !!(user.perms?.nhanSuManage || user.perms?.orgChartManage);
}
function canAccessHrSubTab(user, subTab) {
  if (!user) return false;
  if (user.perms?.admin) return true;
...
// Cơ Cấu Tổ Chức — sub-tab "🌳 Cơ Cấu Tổ Chức" (#hrSubOrgChart) render TRONG #hrSection (module Nhân
// Sự dùng chung 1 gốc cho mọi sub-tab, kể cả #hrSubFeedback sau này khi chuyển tới lượt) — modal "Đổi
// Quản Lý Trực Tiếp" (#orgChartManagerModal) sống NGOÀI section (giống các modal khác) nên cần thêm 1
// gốc riêng — 2 gốc tổng cộng.
