...
//
// LỖI ĐÃ VÁ (đợt audit chuyên sâu cụm "Hệ Thống/Admin/Cấu Hình", mức Trung bình): phần CÒN LẠI của cấu
// hình (baseUrl của hệ thống nội bộ dsmart16, TÊN header xác thực, chu kỳ đồng bộ, trạng thái/thông
// điệp lần đồng bộ gần nhất) trước đây trả cho MỌI tài khoản đã đăng nhập qua GET /api/data — dù màn
// cấu hình chỉ admin mới mở được. Đây là thông tin hạ tầng nội bộ (đích tích hợp + tên header cần gửi)
// chỉ có giá trị cho người tấn công, nhân viên thường không dùng tới ở bất kỳ màn nào. Ẩn HOÀN TOÀN với
// non-admin, cùng khuôn sanitizeExternalApiKeys()/sanitizeAttendanceClockApiKeys() ở trên.
