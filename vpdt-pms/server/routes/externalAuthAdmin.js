...
const { generateApiKey, keyDisplayPrefix, hashApiKey } = require('../lib/externalAuth');
...
// POST /api/admin/external-api-keys — tạo key mới. Body { name }. Trả về "apiKey" (giá trị THẬT) ĐÚNG 1
// LẦN duy nhất ở response này — sau đó DB chỉ còn giữ bcrypt hash, không có cách nào lấy lại được nữa
// (mất thì phải thu hồi + tạo key mới, không có API "xem lại").
router.post('/', async (req, res) => {
  try {
    ...
    insertSystemLog({
      ...
      description: `Tạo API key mới cho ứng dụng ngoài "${name}" (id #${record.id}, prefix ${record.keyPrefix}…)`,
      status: 'SUCCESS'
